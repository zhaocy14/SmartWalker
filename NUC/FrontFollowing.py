    ...
    while True:
        ...
        if backward_boundry > current_position[4] > -40:
            ...
            if current_position[5] > center_left_boundry \
                ...
                # CD.omega = 0.15
                # CD.radius = 80
                CD.omega = 0.1*abs((1+(current_position[1] -left_boundry)*0.3))
                CD.radius= 80*abs((1-(current_position[1] -left_boundry)*0.02))
                str1 = "left"
                time.sleep(0.2)
                ...
                # CD.omega = -0.15
                # CD.radius = 80
                CD.omega = -0.1*abs((1+(right_boundry-current_position[3] )*0.3))
                CD.radius = 80*abs((1-(right_boundry-current_position[3] )*0.02))
                str1 = "right"
                time.sleep(0.2)
            else:
                CD.speed = 0.1
