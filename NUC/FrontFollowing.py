...
def main_FFL(CD: cd.ControlDriver, LD: Leg_detector.Leg_detector):
    ...
    while True:
        ...
        forward_boundry = 4
        backward_boundry = -8
        center_left_boundry = 0.5
        center_right_boundry = 0.5
        left_boundry = 8
        right_boundry = -7
        if backward_boundry > current_position[4] > -40:
            ...
            if current_position[5] > center_left_boundry \
                ...
                # CD.omega = 0.15
                # CD.radius = 80
                CD.omega = 0.13*abs((1+(current_position[1] -left_boundry)*0.3))
                CD.radius= 80*abs((1-(current_position[1] -left_boundry)*0.05))
                str1 = "left"
                time.sleep(0.2)
                ...
                # CD.omega = -0.15
                # CD.radius = 80
                CD.omega = -0.13*abs((1+(right_boundry-current_position[3] )*0.3))
                CD.radius = 80*abs((1-(right_boundry-current_position[3] )*0.05))
                str1 = "right"
                time.sleep(0.2)
            else:
                CD.speed = 0.1
