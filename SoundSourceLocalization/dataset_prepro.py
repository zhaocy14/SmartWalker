...
def segment_audio(dataset_root, dir_name, seg_len, overlap_per=0., fs=16000, threshold=None):
    ...
    for file in files:
        ...
        # segment the audio
        audio_segmenter_4_file(file, save_dpath, segment_len=seg_len, overlap_per=overlap_per, fs=fs, dropout=True,
                               threshold=threshold)
...
def denoise_audio_clips(dataset_root, dir_name, ):
    ...
    model, _ = load_onnx_model(model_path='model/ns_nsnet2-20ms-baseline.onnx')
