...
class LambdaLayerX(tf.keras.layers.Layer):
    def call(self, inputs):
        return inputs


class LambdaLayerLogMax(tf.keras.layers.Layer):
    def __init__(self, log_epsilon, **kwargs):
        super(LambdaLayerLogMax, self).__init__(**kwargs)
        self.log_epsilon = log_epsilon
    
    def call(self, inputs):
        return tf.math.log(tf.math.maximum(inputs, self.log_epsilon))
...
class SpeechFeatures(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.params[
            ...
            self.rand_shift = LambdaLayerX()
        
        if self.params[
            'fe_resample'] != 0.0 and self.mode == modes.Modes.TRAINING:
            self.rand_stretch_squeeze = random_stretch_squeeze.RandomStretchSqueeze(
                self.params['fe_resample'])
        else:
            self.rand_stretch_squeeze = LambdaLayerX()
        ...
        if self.params['use_spec_augment'] and self.mode == modes.Modes.TRAINING:
            ...
            self.spec_augment = LambdaLayerX()
    ...
    def get_params(flags):
        ...
        if flags.time_shift_ms != 0.0 and flags.fe_time_shift_ms != 0.0:
            raise ValueError('both time_shift_ms and fe_time_shift_ms are set '
                             'only one parameter should be used: '
                             'time_shift_ms is used during data reading '
                             'fe_time_shift_ms is used in speech feature extraction '
                             'both of them do random shifts of audio data in time')
        
        if flags.speed_resample != 0.0 and flags.fe_resample != 0.0:
            raise ValueError('both resample and fe_resample are set '
                             'only one parameter should be used: '
                             'resample is used during data reading '
                             'fe_resample is used in speech feature extraction '
        ...
        params = {
            ...
            'sp_time_shift_samples'          :
                int((flags.fe_time_shift_ms * flags.sample_rate) / 1000),
            'fe_resample'                    :
                flags.fe_resample,
            'use_one_step'                   :
                flags.data_stride <= 1,
