...
class AudioProcessor(object):
    ...
    def __init__(self, flags):
        ...
        if flags.data_dir:
            ...
            if flags.split_data:
                ...
                raise ValueError('split_data: Loading data prepared by user has not been implemented yet')
    ...
    def prepare_data_index(self, silence_percentage, unknown_percentage,
        ...
        for wav_path in gfile.Glob(search_path):
            ...
            if word == du.BACKGROUND_NOISE_DIR_NAME:
                continue
            all_words[word] = True
            set_index = du.which_set(wav_path, validation_percentage, testing_percentage)
    ...
    def get_data(self, how_many, offset, flags, background_frequency,
        ...
        pick_deterministically = (mode != 'training') or flags.pick_deterministically
        ...
        for i in xrange(offset, offset + sample_count):
            ...
            if use_background:
                ...
                background_offset = np.random.randint(0, len(background_samples) - flags.desired_samples)
    ...
    def get_unprocessed_data(self, how_many, flags, mode):
        ...
        with tf.Session(graph=tf.Graph()) as sess:
            ...
            for i in range(sample_count):
                ...
                sample = candidates[sample_index]
                input_dict = {
                    wav_filename_placeholder: sample['file']
                }
                if sample['label'] == du.SILENCE_LABEL:
    ...
    def maybe_download_and_extract_dataset(self, data_url, dest_directory):
        ...
        if not os.path.exists(filepath):
            ...
            logging.info('Successfully extract all files')
    ...
    def prepare_processing_graph(self, flags):
        ...
        with tf.get_default_graph().name_scope('data'):
            ...
            # 重采样 -> 变换回原来的大小 -> 音量调整 -> 平移 —> 偏置 -> 选取指定大小声音 -> 噪音（音量调整） -> 输出未提取特征的音频
            if self.foreground_resampling_placeholder_ != 1.0:
                image = tf.expand_dims(wav_data, 0)
                image = tf.expand_dims(image, 2)
                shape = tf.shape(wav_data)
                image_resized = tf.image.resize(images=image,
                                                size=(tf.cast((tf.cast(shape[0], tf.float32) *
                                                               self.foreground_resampling_placeholder_), tf.int32), 1),
                                                preserve_aspect_ratio=False)
                image_resized_cropped = tf.image.resize_with_crop_or_pad(image_resized,
                                                                         target_height=desired_samples,
                                                                         target_width=1, )
                image_resized_cropped = tf.squeeze(image_resized_cropped, axis=[0, 3])
                scaled_foreground = tf.multiply(image_resized_cropped, self.foreground_volume_placeholder_)
            else:
                scaled_foreground = tf.multiply(wav_data, self.foreground_volume_placeholder_)
