...
def update_flags(flags):
    ...
    label_count = len(du.prepare_words_list(flags.wanted_words.split(','), flags.split_data))
    desired_samples = int(flags.sample_rate * flags.clip_duration_ms / MS_PER_SECOND)
    window_size_samples = int(flags.sample_rate * flags.window_size_ms / MS_PER_SECOND)
    window_stride_samples = int(flags.sample_rate * flags.window_stride_ms / MS_PER_SECOND)
    ...
    # if flags.train and (not flags.restore_checkpoint):
    #     upd_flags.train_dir = os.path.join(flags.train_dir, time.strftime("%Y%m%d-%H%M%Sd"))
    upd_flags.summaries_dir = os.path.join(flags.train_dir, 'logs/')
    upd_flags.restore_dir = os.path.join(flags.train_dir, 'restore/')
